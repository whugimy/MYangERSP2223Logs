...
## Goals
- [x] Write goals 
- [ ] Write down teaching assignment due Wednesday
- [ ] Meeting with grad student advisor (Roman) on Friday (have to confirm meeting time)

## Teaching Assignment 10/12/2022
My chosen topic is: "what is dynamic programming."

Broad definition: breaking down complex problem into subproblems, then using those subproblems' solutions to solve the greater problem.

### Example: Fibonacci
When you're first introduced to fibonacci, it's usually done through the following recursion.
...
```
That being said, this recursive method is ineffective, as we will have to run multiple calculations of the same value. More specifically, say you run ```fibonacci(4)```. The function will then proceed to recursively call ```fibonacci(3)``` and ```fibonacci(2)```. A call to ```fibonacci(3)``` will then result in a duplicate call to ```fibonacci(2)```.

An alternate solution to fibonacci is to store computations we've already completed, then, whenever we call fibonacci on a value we've already seen, we can do an ```O(1)``` call to to our precomputed value instead of recursively recomputing the fibonacci of that value all the way back to 1. Imagine we have an array of size ```n``` that we can use to compute the ```n```'th fibonacci number. The implemention would be:

```
arr[] is an array of size n
arr[0] and arr[1] (our base cases) are initialized to 1
for (int i from 2 to n) {
  arr[i] = arr[i-1] + arr[i-2]
}
return arr[n-1]
```

This solution runs in O(n) time and has no duplicate calculations; every next array entry is simply the sum of the previous two. This process of solving the smaller problems, then storing their solutions, then referring back to those stored solutions to solve further problems, is dynamic programming. It should be noted that the above dp fibonacci can be further optimized to constant space with the insight that only the most recent two fibonacci values are ever necessary, but this paradigm of problem solving is dp.
